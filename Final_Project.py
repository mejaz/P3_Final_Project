...
# import dict2xml for converting json to xml

from dict2xml import dict2xml as toXML
...
def displayCatalog():
    ...
    all_items = \
        session.query(Items).order_by(Items.item_id.desc()).limit(9)
    ...
    if 'username' not in login_session:
        ...
        return render_template('catalogonLogin.html',
                               all_catagories=all_catagories,
                               all_items=all_items,
...
def displayCatagoryItems(catagory_name):
    ...
    if 'username' not in login_session:
        catagory_items = \
            session.query(Items).filter_by(catagory_id_fk=sel_cat.catagory_id).all()
        return render_template('catagoryItems.html',
                               all_catagories=all_catagories,
                               all_items=catagory_items,
                               sel_cat=sel_cat)
    else:
        catagory_items = \
            session.query(Items).filter_by(catagory_id_fk=sel_cat.catagory_id,
                user_id_fk=login_session['email']).all()
        return render_template('catagoryItems.html',
                               all_catagories=all_catagories,
                               all_items=catagory_items,
                               sel_cat=sel_cat,
...
def displayItemDesc(catagory_name, userid, item_name):
    ...
    if 'username' not in login_session:
        ...
        sel_cat = \
            session.query(Items).filter_by(catagory_id_fk=catagory.catagory_id,
                item_name=item_name, user_id_fk=userid).one()
        ...
        sel_cat = \
            session.query(Items).filter_by(catagory_id_fk=catagory.catagory_id,
                item_name=item_name, user_id_fk=login_session['email'
                ]).one()
        return render_template('itemDesconLogin.html',
                               catagory_name=catagory_name,
                               item=sel_cat)
...
@app.route('/catalog/<string:catagory_name>/items/XML')
def allItemsXML(catagory_name):
    """ returns an XML"""
